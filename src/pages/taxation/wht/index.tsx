...
// ───────── Tables ─────────
function WhtReceivableTable({ year, period }: Readonly<{ year: number; period: Period }>) {
...
function WhtPayableTable({ year, period }: Readonly<{ year: number; period: Period }>) {
    const [q, setQ] = useState(""); const [page, setPage] = useState(1); const PAGE = 10;
...
function CertificateRegister({ year, period }: Readonly<{ year: number; period: Period }>) {
    const rows = useMemo(
