                            ...
                            <ResponsiveContainer>
                                <BarChart data={waterfall} layout="vertical" margin={{ top: 10, right: 50, left: 100, bottom: 0 }}>
                                    ...
                                    <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                                        {waterfall.map((w) => <Cell key={w.label} fill={w.color} />)}
                            ...
                            <ResponsiveContainer>
                                <PieChart>
                                    <Pie data={expenseBreakdown} dataKey="value" nameKey="name"
                                        ...
                                        {expenseBreakdown.map((e) => <Cell key={e.name} fill={PIE[expenseBreakdown.indexOf(e) % PIE.length]} />)}
                                    </Pie>
                                    <RTooltip content={<MoneyTooltip />} />
                                </PieChart>
                            </ResponsiveContainer>
...
function NumInput({ value, onChange }: Readonly<{ value: number; onChange: (v: number) => void }>) {
    return (
        <input
            type="number"
            value={value === 0 ? "" : value}
